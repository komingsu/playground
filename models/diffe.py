...
class ConditionalUNet(nn.Module):
    def __init__(self, in_channels, n_feat=256, kernel_size=1):
        ...
        self.down1 = UnetDown(in_channels, self.d1_out, kernel_size=kernel_size, gn=8, factor=2)  # 2500 → 1250
        self.down2 = UnetDown(self.d1_out, self.d2_out, kernel_size=kernel_size, gn=8, factor=2)  # 1250 → 625
        self.down3 = UnetDown(self.d2_out, self.d3_out, kernel_size=kernel_size, gn=8, factor=2)  # 625 → 312

        # 업샘플링 레이어
        self.up2 = UnetUp(self.d3_out, self.u2_out, kernel_size=kernel_size, gn=8, factor=2)
        self.up3 = UnetUp(self.u2_out + self.d2_out, self.u3_out, kernel_size=kernel_size, gn=8, factor=2)
        self.up4 = UnetUp(self.u3_out + self.d1_out, self.u4_out, kernel_size=kernel_size, gn=8, factor=2)
        
        self.out = nn.Conv1d(self.u4_out + in_channels, in_channels, 1)

    def forward(self, x, t):
        down1 = self.down1(x)  # 1250 -> 625
        down2 = self.down2(down1)  # 625 -> 312
        down3 = self.down3(down2)  # 312 -> 156

        temb = self.sin_emb(t).view(-1, self.n_feat, 1)

        up1 = self.up2(down3, target_size=down2.shape[-1])  # 🔥 312 -> 625 (down2에 맞춘다)
        up2 = self.up3(torch.cat([up1 + temb, down2], dim=1), target_size=down1.shape[-1])  # 🔥 625 -> 1250 (down1에 맞춘다)
        up3 = self.up4(torch.cat([up2 + temb, down1], dim=1), target_size=x.shape[-1])  # 🔥 1250 -> 2500 (x에 맞춘다)
...
class Encoder(nn.Module):
    def __init__(self, in_channels, dim=512, kernel_size=1):
        ...
        self.down1 = UnetDown(in_channels, self.e1_out, kernel_size=kernel_size, gn=8, factor=2)
        self.down2 = UnetDown(self.e1_out, self.e2_out, kernel_size=kernel_size, gn=8, factor=2)
        self.down3 = UnetDown(self.e2_out, self.e3_out, kernel_size=kernel_size, gn=8, factor=2)
...
class Decoder(nn.Module):
    def __init__(self, in_channels, n_feat=256, kernel_size=1, encoder_dim=512, n_classes=4):
        ...
        self.up1 = UnetUp(self.d3_out + self.e3_out, self.u2_out, kernel_size=kernel_size, gn=8, factor=2)
        self.up2 = UnetUp(self.d2_out + self.u2_out, self.u3_out, kernel_size=kernel_size, gn=8, factor=2)
    ...
    def forward(self, x0, encoder_out, diffusion_out):
        ...
        concat_feat = torch.cat([self.pool(x0), self.pool(x_hat.detach()), up2, dn11.detach()], 1)

        # Convolution 만들기 (여기서 in_channels 자동 계산)
        conv_up3 = nn.Conv1d(
            in_channels=concat_feat.size(1),  # 동적으로 입력 채널 수 잡기
        ...
        out = conv_up3(concat_feat)
        out = F.interpolate(out, size=x0.shape[-1], mode='nearest')  # 🔥 추가
...
class DiffE(nn.Module):
    ...
    def forward(self, x_encoder_input, x_decoder_input, ddpm_out):
        encoder_out = self.encoder(x_encoder_input)  
        decoder_out = self.decoder(x_decoder_input, encoder_out, ddpm_out)
        fc_out = self.fc(encoder_out[1])
        return decoder_out, fc_out, encoder_out  # 🔥 encoder_out 추가
...
class DecoderNoDiff(nn.Module):
    def __init__(self, in_channels, n_feat=256, kernel_size=1, encoder_dim=512, n_classes=4):
        ...
        # Unet up sampling
        self.up2 = UnetUp(self.e3_out, self.u2_out, kernel_size=kernel_size, gn=8, factor=2)
        self.up3 = UnetUp(self.e2_out + self.u2_out, self.u3_out, kernel_size=kernel_size, gn=8, factor=2)
